...
import { Power, RefreshCw, Wifi, WifiOff } from "lucide-react";
    ...
    <div className="min-h-screen flex flex-col">
        ...
        {isAutoPilot && (
          ...
          >
            <div className="max-w-[1800px] mx-auto px-8 py-2 flex items-center gap-3 text-xs text-emerald-300">
              ...
              <span className="text-emerald-500/60 hidden md:inline">— Gemini AI is autonomously monitoring all 15 vessels. Disruptions are automatically resolved without human intervention.</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <main className="flex-1 max-w-[1800px] mx-auto w-full px-4 md:px-8 py-5 flex flex-col gap-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="lg:col-span-2"><DashboardMap state={state} /></div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            { label: "CO₂ Prevented", value: `${(state?.total_co2_saved_tons || 0).toLocaleString()} t`, sub: "metric tons saved" },
            { label: "System Uptime", value: metrics?.uptime_seconds ? `${Math.floor(metrics.uptime_seconds / 60)}m ${Math.floor(metrics.uptime_seconds % 60)}s` : "—", sub: "continuous monitoring" },
          ].map((item, i) => (
