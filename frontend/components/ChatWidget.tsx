...
export default function ChatWidget() {
  ...
  const [messages, setMessages] = useState<Msg[]>([{
    role: "ai",
    text: "👋 I'm your **AI Supply Chain Assistant** powered by Gemini 2.0 Flash. Ask me anything about live fleet data — or tap **Exec Summary** for a board-level briefing.",
    ts: now(),
  }]);
  ...
  /* Auto-open once per session so the AI is visible without needing to find the button */
  useEffect(() => {
    const already = sessionStorage.getItem("chat_auto_opened");
    if (!already) {
      const t = setTimeout(() => {
        setIsOpen(true);
  ...
  return (
    <>
      {/* ── FAB — always-visible "Ask AI" pill ── */}
      <motion.button
        onClick={() => setIsOpen(v => !v)}
        whileHover={{ scale: 1.04 }}
        whileTap={{ scale: 0.96 }}
        className="fixed bottom-6 right-6 z-50 h-13 rounded-full flex items-center gap-2.5 shadow-2xl"
        style={{
          background: isOpen
            ? "rgba(15,18,40,0.95)"
            : "linear-gradient(135deg, #3b82f6, #6366f1)",
          boxShadow: isOpen
            ? "0 0 0 1px rgba(99,102,241,0.4), 0 8px 24px rgba(0,0,0,0.5)"
            : "0 0 30px rgba(99,102,241,0.55), 0 8px 24px rgba(0,0,0,0.4)",
          padding: isOpen ? "0 18px" : "0 20px",
          height: "52px",
        ...
        <AnimatePresence mode="wait">
          {isOpen ? (
            <motion.div key="close" initial={{ rotate: -90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: 90, opacity: 0 }}
              ...
              <span className="text-sm font-semibold text-gray-300">Close</span>
            </motion.div>
          ) : (
            <motion.div key="open" initial={{ rotate: 90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }}
              ...
              <div className="relative">
                ...
                <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-[#4f46e5] animate-pulse" />
              </div>
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-b border-white/8 shrink-0"
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[11px] text-emerald-400 flex items-center gap-1">
                  <Sparkles size={9} /> Gemini 2.0 Flash · Live Data
                </p>
              </div>
              <button onClick={handleExecutiveSummary} disabled={summaryLoading}
                className="flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1.5 rounded-lg border border-purple-500/30 bg-purple-500/10 text-purple-300 hover:bg-purple-500/20 transition-all disabled:opacity-50 shrink-0">
                <FileText size={11} />
                {summaryLoading ? "Generating..." : "Exec Summary"}
              </button>
              <button onClick={() => setIsOpen(false)}
                className="p-1.5 rounded-lg hover:bg-white/10 text-gray-500 hover:text-white transition-colors ml-1 shrink-0">
                <X size={15} />
              </button>
            </div>
              ...
              <div className="px-4 pb-2 flex flex-wrap gap-1.5">
                {SUGGESTIONS.map(s => (
                  <button key={s} onClick={() => send(s)}
                    className="text-[10px] px-2.5 py-1.5 rounded-lg border border-blue-500/20 bg-blue-500/8 text-blue-300 hover:bg-blue-500/18 transition-all text-left">
                    {s}
                  </button>
                ))}
              </div>
